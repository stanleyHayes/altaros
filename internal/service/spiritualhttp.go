...
func handleTodayDevotional(s *spiritual.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		item, err := s.Today(r.Context())
		if err != nil {
...
func handleDevotionals(s *spiritual.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		p, l := pageParams(r)
		items, total, err := s.Devotionals(r.Context(), p, l)
		if err != nil {
			writeSpiritualError(w, err)
			return
		}
...
func handleSermons(s *spiritual.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		p, l := pageParams(r)
		items, total, err := s.Sermons(r.Context(), p, l, r.URL.Query().Get("series"))
		if err != nil {
			writeSpiritualError(w, err)
			return
		}
...
func handleSermon(s *spiritual.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		item, err := s.Sermon(r.Context(), chi.URLParam(r, "id"))
		if err != nil {
			writeSpiritualError(w, err)
			return
		}
...
func handlePrayers(s *spiritual.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		p, l := pageParams(r)
		items, total, err := s.Prayers(r.Context(), p, l)
		if err != nil {
			writeSpiritualError(w, err)
			return
		}
...
func handleCreatePrayer(s *spiritual.Service, members *member.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var body struct {
		...
		m, err := members.ByUserID(r.Context(), callerUserID(r))
		if err != nil {
			writeSpiritualError(w, spiritual.ErrMemberRequired)
			return
		}
...
func handlePray(s *spiritual.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		item, err := s.Pray(r.Context(), chi.URLParam(r, "id"))
		if err != nil {
			writeSpiritualError(w, err)
			return
		}
...
func writeSpiritualError(w http.ResponseWriter, err error) {
	switch {
	case errors.Is(err, spiritual.ErrNotFound):
		httpx.Error(w, http.StatusNotFound, "That spiritual resource is not available.")
	case errors.Is(err, spiritual.ErrInvalidInput):
		httpx.Error(w, http.StatusBadRequest, "Check the prayer title and description.")
	case errors.Is(err, spiritual.ErrMemberRequired):
