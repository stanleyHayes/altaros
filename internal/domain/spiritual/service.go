...
func (s *Service) CreatePrayer(ctx context.Context, in PrayerInput) (*PrayerRequest, error) {
	...
	if in.Title == "" || len(in.Title) > 160 || in.Description == "" || len(in.Description) > 4000 {
	...
	if !in.IsAnonymous && strings.TrimSpace(in.AuthorName) != "" {
		doc["authorName"] = strings.TrimSpace(in.AuthorName)
	}
	res, err := s.prayers.InsertOne(ctx, doc)
	if err != nil {
		return nil, err
	}
	var out PrayerRequest
	if err := s.prayers.FindOne(ctx, bson.M{"_id": res.InsertedID}, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
func (s *Service) Pray(ctx context.Context, id string) (*PrayerRequest, error) {
	...
	var out PrayerRequest
	if err := s.prayers.FindOne(ctx, bson.M{"_id": oid}, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
