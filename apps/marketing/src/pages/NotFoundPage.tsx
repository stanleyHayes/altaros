  ...
  return (
    <Box component="main" sx={{ minHeight: "min(78dvh, 820px)", display: "grid", placeItems: "center", px: { xs: 2.5, md: 5 }, py: { xs: 7, md: 10 }, bgcolor: "#F3F8F6", color: "#0C302C", overflow: "hidden", position: "relative" }}>
      ...
      <Box sx={{ width: "min(100%, 1040px)", display: "grid", gridTemplateColumns: { xs: "1fr", md: "minmax(280px,.78fr) minmax(360px,1.22fr)" }, gap: { xs: 5, md: 10 }, alignItems: "center", position: "relative" }}>
        ...
        <Box>
          <Typography sx={{ color: "#157F73", fontSize: ".76rem", textTransform: "uppercase", letterSpacing: ".18em", fontWeight: 750 }}>Altar OS</Typography>
          <Typography component="h1" sx={{ mt: 2.25, fontSize: "clamp(2.7rem,6vw,5.4rem)", fontWeight: 800, letterSpacing: "-.065em", lineHeight: .95, maxWidth: 660, textWrap: "balance" }}>We could not find that doorway.</Typography>
          <Typography sx={{ mt: 3, color: "#58706C", fontSize: { xs: "1rem", md: "1.12rem" }, lineHeight: 1.75, maxWidth: 560, textWrap: "pretty" }}>The address may be incomplete or the page may have moved. Start again from the platform overview, or return to where you came from.</Typography>
          <Box sx={{ mt: 4.5, display: "flex", flexWrap: "wrap", gap: 1.5 }}>
            <Button variant="contained" startIcon={<HomeRounded />} onClick={() => navigate("/")} sx={{ px: 3, py: 1.4, bgcolor: "#157F73", "&:hover": { bgcolor: "#0F695F", transform: "translateY(-2px)" }, transition: "transform 180ms ease, background-color 180ms ease" }}>Return to Altar OS</Button>
