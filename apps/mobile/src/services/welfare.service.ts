...
export const WELFARE_HISTORY_PAGE_SIZE = 25;
const MAX_ID_LENGTH = 128;
const SAFE_ID = /^[A-Za-z0-9][A-Za-z0-9._:-]*$/;

export interface WelfareHistoryPage {
...
export function normalizeEmergencyAcknowledgement(
  ...
  const payload = unwrapApiData(value, 'The server did not acknowledge this urgent request.');
  if (typeof payload !== 'object' || payload === null || Array.isArray(payload)) {
    throw new Error('The server did not acknowledge this urgent request.');
  }
  ...
  if (!valid) throw new Error('The server did not acknowledge this urgent request.');
  return {
...
const welfareService = {
  async listMinePage(
    ...
    memberId: string,
    page: number,
    limit = WELFARE_HISTORY_PAGE_SIZE,
  ): Promise<WelfareHistoryPage> {
    if (!validId(churchId) || !validId(memberId)) {
      throw new Error('The member identity is invalid.');
    }
    if (!Number.isSafeInteger(page) || page < 1
      || !Number.isSafeInteger(limit) || limit < 1 || limit > 50) {
      throw new Error('The welfare history page is invalid.');
    }
    const { data } = await api.get<unknown>('/welfare/my-requests', { params: { page, limit } });
    const payload = unwrapApiData(data, 'The server returned invalid welfare history.');
    if (typeof payload !== 'object' || payload === null || Array.isArray(payload)) {
      throw new Error('The server returned invalid welfare history.');
    }
    ...
    const requests = candidate.data.map((request) => normalizeWelfareRequest(request, churchId, memberId));
    ...
    return { requests, total: Number(candidate.total) };
  },
